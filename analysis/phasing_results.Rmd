...
Also load the distances between first and last heterozygote in each pseudo-diploid:
...
These figures are wrong -> they double count flips 
...
```{r}
df_wh_si %>%
  mutate(total_errors_mb = total_errors / size_mb) %>%
  select(id, pop, total_errors_mb) %>%
  rename(shapeit = total_errors_mb) %>%
  inner_join({df_wh_eagle %>% 
      mutate(total_errors_mb = total_errors / size_mb) %>%
      select(id, pop, total_errors_mb) %>% rename(eagle = total_errors_mb)}, by = c("id", "pop")) %>%
  ggplot(aes(x = shapeit, y = eagle, colour = pop)) + 
  geom_point() +
  geom_abline(slope = 1, intercept = 0) +
  ggtitle("Total Errors per MB") +
  xlab("SHAPEIT Errors") +
  ylab("Eagle Errors") +
  guides(colour=guide_legend(title="Population")) +
  scale_color_manual(values = cbPalette) +
...
df_wh_si %>%
  ...
  inner_join({df_wh_beagle %>%
      mutate(total_errors_mb = total_errors / size_mb) %>%
      select(id, pop, total_errors_mb) %>% rename(beagle = total_errors_mb)}, by = c("id", "pop")) %>%
  ggplot(aes(x = shapeit, y = beagle, colour = pop)) + 
  geom_point() +
  geom_abline(slope = 1, intercept = 0) +
  ggtitle("Total Errors") +
...
df_wh_eagle %>%
  ...
  scale_color_manual(values = cbPalette) +
  theme_bw(base_size = 16) +
  theme(legend.position = c(0.8, .29))
```
...
df_wh_si %>%
  ...
  inner_join({df_wh_eagle %>% 
      select(id, pop, switches) %>% rename(eagle = switches)}, by = c("id", "pop")) %>%
  ggplot(aes(x = shapeit, y = eagle, colour = pop)) + 
  geom_point() +
  geom_abline(slope = 1, intercept = 0) +
  ggtitle("Switches") +
  xlab("SHAPEIT Switches") +
  ylab("Eagle Switches") +
  guides(colour=guide_legend(title="Population")) +
  scale_color_manual(values = cbPalette) +
  theme_bw(base_size = 16) +
...
```{r}
df_wh_si %>%
  mutate(switches_mb = switches / size_mb) %>%
  select(id, pop, switches_mb) %>%
  rename(shapeit = switches_mb) %>%
  inner_join({df_wh_eagle %>% 
      mutate(switches_mb = switches / size_mb) %>%
      select(id, pop, switches_mb) %>% rename(eagle = switches_mb)}, by = c("id", "pop")) %>%
  ggplot(aes(x = shapeit, y = eagle, colour = pop)) + 
  geom_point() +
  geom_abline(slope = 1, intercept = 0) +
  ggtitle("Switches per MB") +
  xlab("SHAPEIT Switches") +
  ylab("Eagle Switches") +
  guides(colour=guide_legend(title="Population")) +
  scale_color_manual(values = cbPalette) +
...
df_wh_si %>%
  ...
  inner_join({df_wh_beagle %>% 
      mutate(switches_mb = switches / size_mb) %>%
      select(id, pop, switches_mb) %>% rename(beagle = switches_mb)}, by = c("id", "pop")) %>%
  ggplot(aes(x = shapeit, y = beagle, colour = pop)) + 
  geom_point() +
  geom_abline(slope = 1, intercept = 0) +
  ggtitle("Switches per MB") +
...
df_wh_si %>%
  ...
  xlab("SHAPEIT Error Rate") +
  ylab("Eagle Error Rate") +
  guides(colour=guide_legend(title="Population")) +
  scale_color_manual(values = cbPalette) +
  theme_bw(base_size = 16) +
...
```{r}
df_wh_si %>%
  mutate(flips_mb = flips / size_mb) %>%
  select(id, pop, flips_mb) %>%
  rename(shapeit = flips_mb) %>%
  inner_join({df_wh_eagle %>%
      mutate(flips_mb = flips / size_mb) %>%
      select(id, pop, flips_mb) %>% rename(eagle = flips_mb)}, by = c("id", "pop")) %>%
  ggplot(aes(x = shapeit, y = eagle, colour = pop)) + 
  geom_point() +
  geom_abline(slope = 1, intercept = 0) +
  xlab("SHAPEIT flips per MB") +
  ylab("Eagle flips per MB") +
  guides(colour=guide_legend(title="Population")) +
  scale_color_manual(values = cbPalette) +
  theme_bw(base_size = 16) +
  guides(colour="none")
...
df_wh_si %>%
  ...
  inner_join({df_wh_beagle %>%
      mutate(flips_mb = flips / size_mb) %>%
      select(id, pop, flips_mb) %>% rename(beagle = flips_mb)}, by = c("id", "pop")) %>%
  ggplot(aes(x = shapeit, y = beagle, colour = pop)) + 
  geom_point() +
  geom_abline(slope = 1, intercept = 0) +
  xlab("SHAPEIT flips per MB") +
  ylab("Beagle flips per MB") +
  guides(colour=guide_legend(title="Population")) +
  scale_color_manual(values = cbPalette) +
  theme_bw(base_size = 16) +
  guides(colour="none")
...
df_wh_si %>%
  ...
  xlab("SHAPEIT Switch Rate") +
  ylab("Eagle Switch Rate") +
  guides(colour=guide_legend(title="Population")) + 
  scale_color_manual(values = cbPalette) +
  theme_bw(base_size = 16) +
...
df_vcftools %>% 
  ...
  geom_abline(slope = 1, intercept = 0) +
  theme_bw(base_size = 12) +
  theme(legend.position = c(0.79, .3)) +
  guides(colour=guide_legend(title="Population")) +
  scale_colour_manual(values=cbPalette)
```
...
df_vcftools %>% 
  ...
  geom_abline(slope = 1, intercept = 0) +
  theme_bw(base_size = 12) +
  theme(legend.position = c(0.88, .72)) +
  guides(colour=guide_legend(title="Population")) +
  scale_colour_manual(values=cbPalette)
```
...
```{r}
df_vcftools %>%
  mutate(prop_error_eagle = (n_flip_cpg_eagle + n_other_cpg_eagle) / (n_flip_eagle + n_other_eagle),
         prop_error_shapeit = (n_flip_cpg_shapeit + n_other_cpg_shapeit) / (n_flip_shapeit + n_other_shapeit),
         prop_error_beagle = (n_flip_cpg_beagle + n_other_cpg_beagle) / (n_flip_beagle + n_other_beagle)) %>%
...
df_vcftools %>%
  mutate(prop_error_eagle = n_flip_cpg_eagle / n_flip_eagle,
         prop_error_shapeit = n_flip_cpg_shapeit / n_flip_shapeit,
         prop_error_beagle = n_flip_cpg_beagle / n_flip_beagle) %>%
  {t.test(.$prop_error_eagle, .$prop_error_beagle)}
...
df_vcftools %>%
  mutate(prop_error_eagle = n_flip_cpg_eagle / n_flip_eagle,
         prop_error_shapeit = n_flip_cpg_shapeit / n_flip_shapeit,
         prop_error_beagle = n_flip_cpg_beagle / n_flip_beagle) %>%
  {t.test(.$prop_error_eagle, .$prop_error_shapeit)}

print("Beagle, Shapeit")
df_vcftools %>%
  mutate(prop_error_eagle = n_flip_cpg_eagle / n_flip_eagle,
         prop_error_shapeit = n_flip_cpg_shapeit / n_flip_shapeit,
         prop_error_beagle = n_flip_cpg_beagle / n_flip_beagle) %>%
  {t.test(.$prop_error_beagle, .$prop_error_shapeit)}
```
...
df_vcftools %>%
  ...
  ggtitle("Chi-Square Test: Beagle", "Proportion of Errors at CpGs = Proportion 1KGP Variants at CpG") +
  xlab("Density") +
...
df_vcftools %>%
  ...
  geom_density() +
  geom_vline(xintercept = qchisq(0.05, df = 1, lower.tail = F)) +
  ggtitle("Chi-Square Test: Eagle", "Proportion of Errors at CpGs = Proportion 1KGP Variants at CpG") +
  xlab("Density") +
  ylab("Chi Square Statistics")
...
df_vcftools %>%
  ...
  geom_density() +
  geom_vline(xintercept = qchisq(0.05, df = 1, lower.tail = F)) +
  ggtitle("Chi-Square Test: SHAPEIT", "Proportion of Errors at CpGs = Proportion 1KGP Variants at CpG") +
  xlab("Density") +
  ylab("Chi Square Statistics")
...
df_vcftools %>%
  ...
  geom_density() +
  geom_vline(xintercept = qchisq(0.05, df = 1, lower.tail = F)) +
  ggtitle("Chi-Square Test: Beagle", "Proportion of Flips at CpGs = Proportion 1KGP Variants at CpG") +
  xlab("Density") +
  ylab("Chi Square Statistics")
...
df_vcftools %>%
  ...
  geom_density() +
  geom_vline(xintercept = qchisq(0.05, df = 1, lower.tail = F)) +
  ggtitle("Chi-Square Test: Eagle", "Proportion of Flips at CpGs = Proportion 1KGP Variants at CpG") +
  xlab("Density") +
  ylab("Chi Square Statistics")
...
df_vcftools %>%
  ...
  geom_density() +
  geom_vline(xintercept = qchisq(0.05, df = 1, lower.tail = F)) +
  ggtitle("Chi-Square Test: SHAPEIT", "Proportion of Flips at CpGs = Proportion 1KGP Variants at CpG") +
  xlab("Density") +
  ylab("Chi Square Statistics")
```
...
tapply(df_pd_maf[df_pd_maf$is_flip,]$maf, df_pd_maf[df_pd_maf$is_flip,]$method, summary)
...
counts_per_bin <- function(ids = 1:200, bin_w = 1000){
  ...
  for(i in 2:length(ids)){
    df_pd_Eagle <-  read_csv(paste0(eagle_switch_dir, "switch_", ids[i], ".csv"), show_col_types = FALSE) 
    df_pd_shapeit2 <- read_csv(paste0(shapeit_switch_dir, "switch_", ids[i], ".csv"), show_col_types = FALSE) 
    df_pd_bEagle <- read_csv(paste0(beagle_switch_dir, "switch_", ids[i], ".csv"), show_col_types = FALSE) 
    ...
    df_pd_eagle <- df_pd_eagle %>%
      full_join(df_pd_Eagle, by = "bin") %>%
      replace_na(list("n.x" = 0, "n.y" = 0)) %>%
      mutate(n = n.x + n.y) %>%
      select(bin, n)
    df_pd_beagle <- df_pd_beagle %>%
      full_join(df_pd_bEagle, by = "bin") %>%
      replace_na(list("n.x" = 0, "n.y" = 0)) %>%
      mutate(n = n.x + n.y) %>%
      select(bin, n)
    df_pd_shapeit <- df_pd_shapeit %>%
