...
#SBATCH --array=1-400
#SBATCH -e /net/snowwhite/home/beckandy/research/phasing/output/gnomad/slurm/pair.%A_%a.err
#SBATCH --output=/net/snowwhite/home/beckandy/research/phasing/output/gnomad/slurm/pair.%A.%a.out

VCF="/net/snowwhite/home/beckandy/research/phasing/data/gnomAD/gnomad.filterPASS.1kgp.nonpar.mask.noSing.bcf"
out_dir="/net/snowwhite/home/beckandy/research/phasing/output/gnomad"
# Read line from sample_pairs.csv
line=$(awk -v id=${SLURM_ARRAY_TASK_ID} 'NR==id{ print; exit }' /net/snowwhite/home/beckandy/research/phasing/data/sample_pairs_gnomad.csv)
arrSub=(${line//,/ })
sub_a=${arrSub[1]}
sub_b=${arrSub[2]}
...
bcftools view -v snps -s $sub_a,$sub_b -Ou $VCF |\
	...
	bcftools view -c 1 -g ^miss -Oz > /tmp/andy_input_${SLURM_ARRAY_TASK_ID}.vcf.gz

# Regen site list (weird thing where initial list doesn't drop all sites with missing values)
# bcftools query  -f '%CHROM\t%POS\n' /tmp/andy_input_${SLURM_ARRAY_TASK_ID}.vcf.gz | tr -s ' ' > $out_dir/site_list/sites_${SLURM_ARRAY_TASK_ID}.tsv
...
bcftools view /tmp/andy_ref_${SLURM_ARRAY_TASK_ID}.bcf -Ov |\
bcftools +missing2ref - -- |\
bcftools +fixploidy - -- |\
sed 's/\//|/g' > /tmp/andy_ref_${SLURM_ARRAY_TASK_ID}.vcf
