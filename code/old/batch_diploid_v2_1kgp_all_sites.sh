...
#SBATCH --array=1
#SBATCH -e /net/snowwhite/home/beckandy/research/phasing/output/nygc/slurm/pair.%A_%a.err
#SBATCH --output=/net/snowwhite/home/beckandy/research/phasing/output/nygc/slurm/pair.%A.%a.out

VCF="/net/snowwhite/home/beckandy/research/phasing/data/1kgp/1kGP_nonPAR_mask_noSing.bcf"
out_dir="/net/snowwhite/home/beckandy/research/phasing/output/nygc/all_sites"
...
bcftools view -v snps -s $sub_a,$sub_b -Ou $VCF |\
	bcftools view -T $out_dir/site_list/sites_${SLURM_ARRAY_TASK_ID}.tsv -Oz > /tmp/andy_input_${SLURM_ARRAY_TASK_ID}.vcf.gz
...
vcftools --vcf /tmp/andy_eagle_${SLURM_ARRAY_TASK_ID}.vcf \
  ...
  --diff-switch-error \
  --out $out_dir/switch_errors/error_${SLURM_ARRAY_TASK_ID}_eagle

vcftools --gzvcf /tmp/andy_shapeit_${SLURM_ARRAY_TASK_ID}.vcf.gz \
  --gzdiff /tmp/andy_input_${SLURM_ARRAY_TASK_ID}_truth.vcf.gz \
  --diff-switch-error \
  --out $out_dir/switch_errors/error_${SLURM_ARRAY_TASK_ID}_shapeit

vcftools --gzvcf /tmp/andy_beagle_${SLURM_ARRAY_TASK_ID}.vcf.gz \
  --gzdiff /tmp/andy_input_${SLURM_ARRAY_TASK_ID}_truth.vcf.gz \
  --diff-switch-error \
  --out  $out_dir/switch_errors/error_${SLURM_ARRAY_TASK_ID}_beagle
